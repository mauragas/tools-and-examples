...
namespace AdAwayHost.Console
{
    public class AppConfiguration
    {
        ...
        private Options ParseCommandArguments(string[] commandLineArguments)
        {
            ...
            CommandLine.Parser.Default.ParseArguments<Options>(commandLineArguments)
        ...
        private string GetHostsFileLocation()
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) hostFileLocation = "/etc/hosts";
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) hostFileLocation = @"C:\Windows\System32\drivers\etc\hosts";
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX)) hostFileLocation = "/private/etc/hosts";

            if (!_fileWrapper.Exists(hostFileLocation))
